...
- **Riga** (`.dt-row`): padding laterale 8vw (5vw sotto i 768 px). Nessun `max-width` generale: il contenuto si ferma per misura di riga (lead 50ch, body 60ch, titoli 16–28ch), non per contenitore. Il solo capitolo con un tetto è Costi (960 px).
- **Capitolo** (`.dt-chapter`): padding verticale `clamp(6rem, 14vh, 11rem)` (mobile `clamp(4rem, 10vh, 6rem)`). Ogni sezione della home è `dt-chapter` su avorio; nessuna banda cambia fondo.
- **Griglia**: due colonne da lg (1024 px) con gutter 6vw, quasi sempre asimmetriche — `5fr 7fr`, `1.2fr 1fr`, `1fr 1.1fr`, `1.25fr 1fr`, `2fr 3fr`, `1fr 1.2fr` — foto da un lato e testo dall'altro, alternate nei percorsi. Le liste vanno a 2 (md) e 3 (lg) colonne con gutter 3vw e 4rem verticali; il footer a `1.4fr 1fr 1fr 1fr` con gap 3rem.
...
- **Media**: sempre a tutta larghezza della colonna, rapporto fisso, `object-cover`, sopra un rettangolo avorio profondo: `aspect-video` per video e banda finale, `aspect-square` per la foto di capitolo, `aspect-[4/5]` per i ritratti; le tessere della rotaia del team a 4/5, 3/2 e 4/3 con larghezze in vw (78/52/34 · 92/78/63.75 · 92/70/56.67 su telefono/tablet/desktop).
- **Testata**: una riga alta `clamp(5rem, 12vh, 7.5rem)` col logo ufficiale a `clamp(150px, 14vw, 220px)`; sticky solo sotto lg (serve al bottone Menu), da lg in su scorre via e si apre su due righe — logo, monogramma e lingua sopra, nove voci maiuscole allineate a destra sotto. Le ancore hanno `scroll-margin-top: 6.5rem`.
- **Elementi fissi**: WhatsApp tondo in basso a destra da sm (640 px) in su; sotto sm la barra azioni (hairline sopra, avorio, CTA piena + WhatsApp 52 px); il footer riserva 7rem + safe-area in fondo. Il banner cookie è fisso, centrato, max 42rem.
...
- **Do** aprire ogni capitolo con la stessa cellula: eyebrow rosso → titolo → (una parola Pinyon rossa) → lead a 300 in grafite ≤ 50ch → link ghost.
