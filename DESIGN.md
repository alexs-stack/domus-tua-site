...
- **Primo schermo della home**: testata chiara; poi la BANDA fotografica alta `--dt-band-h` (60svh) con sopra il solo lockup «Domus Tua» e, a cavallo del suo bordo basso, la firma Pinyon rossa (che sporge del 26% e chiede al blocco sotto l'aria per le sue discendenti); sull'avorio, centrati, sovratitolo, H1 (d3, ≤ 28ch), CTA e voto. La banda è la stessa scatola della sagoma del preloader: vedi **Il patto della porta**.
- **Primo schermo delle pagine interne**: la testa sta su DUE colonne da lg (`1.1fr 1fr`) — occhiello, h1 e calligrafia a sinistra, lead e CTA allineati in basso a destra — e la banda occupa entrambe le colonne nella riga sotto, risalendo sotto la colonna del titolo così che la calligrafia ne attraversi il bordo alto per un terzo. Il blocco di testo sopra la fotografia sta in ~460 px: quando era in colonna unica ne faceva 940, e su una pagina non se ne vedeva un pixel. Sul telefono l'ordine è occhiello, titolo, calligrafia, FOTO, lead, CTA — la fotografia arriva prima del paragrafo, e in colonna 4:5, non in una feritoia 16:9 da 219 px.
...
L'ultima sezione: video 16:9 a tutta larghezza (min 70svh, scala 1.14 — serve a mangiare il logo bruciato nell'angolo alto a sinistra della clip, non è un effetto) con il titolo d1 bianco (≤ 12ch) e un link `ghost-dark` a d4 300 in basso a sinistra, a 12vh dal fondo. Bianco su video senza velo: l'unica volta in cui il testo sta sopra un'immagine. Sul video desktop `object-position` non ha alcun effetto (16:9 dentro 16:9) e nessun taglio può evitare le tende bianche del bordo piscina: la leggibilità la regge un'OMBRA ATTACCATA ALLE LETTERE (`0 1px 2px rgb(0 0 0/.35), 0 0 28px rgb(0 0 0/.45)`), la stessa grammatica della copertina delle cinque stelle. Non è un velo e non è una vignettatura: non c'è nessun rettangolo sopra il video. Il poster (che si vede solo dove il video non parte) non è una fotografia già usata altrove nella pagina.
