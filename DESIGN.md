...
- **Primo schermo della home.** Si apre con la testata chiara. Sotto c'è la banda fotografica a tutta larghezza, alta `--dt-band-h` — **dal 20 set. (A44 di Alberto) lo schermo intero meno la testata**, `calc(100svh − testata − 1px)`, come su era-residence («la foto è il background stesso»); era 60svh e la foto restava col mento di Raffaela sul bordo — su avorio profondo, con la foto dietro il lettering: **la foto ALTA generata con Higgsfield** (A44: la cliente non può più mostrare l'attico di prima; il piano e le 22 generazioni in `docs/superpowers/specs/2026-09-20-foto-alte-higgsfield.md`), il salotto a doppia altezza di una villa con piscina col **ritaglio vero di Raffaela** posato in basso a sinistra da `scripts/media/foto-alte.mjs` — **AMPIA sul desktop** (A45 di Alberto, 21 set.: «non mi piace la foto che hai messo per l'hero: vai a guardare com'era prima nel main, era molto più ampia» — il vecchio scatto era una stanza intera 2000×1415, non una fetta): `hero-raffaela-villa.jpg` è 3:2, 2560×1717, la stanza tutta in campo, Raffaela alta il 55 % del canvas in basso a sinistra; 9:16 sul telefono (`hero-raffaela-villa-m.jpg`, 1440×2580, Raffaela al 42 %), art direction a 768 con `getImageProps` e `<picture>`, `preload`, qualità 78, `object-cover`, `objectPosition: "10% 100%"`, riquadro dello zoom **ancorato in basso** col rapporto della sorgente (`--dt-hero-ar`: 2560/1717 da 768, 1440/2580 sotto): a riposo la banda 16:9 mostra il piede del 3:2 con Raffaela intera (A27), il cielo della vetrata serve agli schermi più alti che larghi. Il tuffo (A23) non fa più salire la foto — scoprirebbe il fondo — e resta il solo zoom a 2. Nessun velo. Il `<video>` si monta solo se è abilitato in `media.ts`, con motion ok e da 768 px. Sulla foto stanno solo due cose (decisione di lavoro, 11 set.: senza velo un H1 a 38 px sopra un divano non si legge, un lockup a 13vw si legge su qualunque stanza): il lockup «Domus Tua» su due righe, centrato in orizzontale e in verticale nella banda, e a cavallo del bordo basso la firma Pinyon rossa (la cliente, 10 set.: «firma più in basso»). La firma sporge del 26 % (`--script-tuck: 0`), e per questo il blocco sotto le lascia l'aria per le discendenti. Sull'avorio seguono, centrati (Alberto, 10 set.: «rimettilo centrale») e ancora nel primo schermo, il sovratitolo (`text-ui` 600 maiuscolo, pietra), l'H1 d3 ≤ 28ch, «Richiedi la valutazione» piena in taglia `lg`, i due link «Vendi casa» → /vendi, al posto del bottone del video (la cliente, 10 set.), e «Cerco casa» → #cerca, e il voto «4,9/5 · 542 recensioni Google» con cinque stelle d'oro da 16 px. Niente subcopy, niente riga del founder. La banda è la stessa scatola della sagoma del preloader: vedi **Il patto della porta**.
- **Primo schermo delle pagine interne: «la testa di era»** (`PageHero` + `PageHeroTesta`; A38, A40 e A41 di Alberto, 20 set.; lo usano 11 pagine, 9 con la foto della villa e 2 legali ferme). La fotografia è il fondo dal primo pixel, sotto la testata trasparente (`header[data-su-foto]`, nav e monogramma bianchi), **ed è la pagina stessa** (A45 di Alberto, 21 set.: «le foto su era residence sono la pagina stessa: quando scrolli, le scritte salgono su come se fossero in quello spazio della foto, e stai scrollando la foto stessa come se fosse la pagina»): il riquadro `.dt-testa_riquadro` è IN FLUSSO, `position: relative`, alto quanto la foto resa a larghezza piena (`aspect-ratio: var(--dt-testa-ar)`, il rapporto della sorgente scritto da `PageHero`; mai meno di 100svh), `overflow: clip`, e scorre con la pagina 1:1 — nessuno sticky, nessun pan, nessuno zoom, nessun pin, nessun ScrollTrigger (lo sticky di A41 e `usePanTesta` di A44 sono morti; `page-dive`, `soglia` e `ingresso` del commit R già prima). **Le nove foto sono ALTE** (A44, 20 set.) — 2:3, 2560×3816, generate con Higgsfield nello stile di era-residence adattato a Tradate (`*-alta.jpg` in `public/images/reali`, `tinte.json`) — così a 1440 il riquadro è alto 2146 px: il primo schermo mostra la CIMA con le scritte (`lg: "50% 0%"`: cielo o soffitto sotto il bianco, regola 1 del piano), e scorrendo la foto continua da sola per un altro schermo e mezzo, come «Perfect sea views» di era, prima che l'avorio riprenda. Sul telefono (`sotto: "50% 50%"`) un 2:3 è più basso dello schermo e vince 100svh. Il blocco `.dt-testa_blocco` sta DENTRO il riquadro, in cima (`position: absolute; inset: 0 0 auto`), alto almeno 100svh, griglia a tre righe centrate come «Perfect sea views»: il lead in alto sotto la testata, occhiello + h1 + calligrafia al centro, i due comandi in basso (bottone rosso pieno, link fantasma bianco). L'h1 ha una misura sua, `clamp(3rem, 9vw, 9rem)` con interlinea 0.92, da lg `clamp(3.5rem, 7vw, 8rem)`, in colonna ≤ 18ch; la calligrafia è bianca. I tre punti di prova seguono la foto sull'avorio (`.dt-testa_pagina`, un passo di capitolo: `clamp(12rem, 30svh, 22rem)`). L'inquadratura (`objectPosition`) è scelta per fascia in `tinte.json` (`lg` / `sotto`, D180) con A27 prima di tutto: nessuna persona tagliata. Il layout è tutto in CSS prima del paint: con reduced-motion e senza JS la pagina è identica. **Deroga di Alberto a WCAG 1.4.3 (A38 + A40, «bianco nudo»)**: le scritte bianche stanno sulla sola fotografia, senza ombra, alone, velo o tinta (A40: «negli screenshot di era-residence non c'erano le ombre sulle scritte bianche, rimuovile subito»). Misurato sui pixel resi (`qualita/a38/inquadrature.md`, 25 inquadrature × 3 viewport): otto rotte su nove non arrivano a 3:1 — h1 1,3-2,0:1, lead 1,1-1,8:1 — e passa solo /metodo (h1 4,17:1); è una scelta dichiarata di Alberto, da mostrare alla cliente, non un difetto da correggere con un velo (C14). Prima che la foto decodifichi il riquadro mostra la tinta alta della rotta (D125); l'e2e di accessibilità aspetta la foto prima di misurare.
...
**La finestra di Open Domus è la sezione «Architecture» di era-residence (A45 di Alberto, 21 set.: «non hai fatto quello che ti ho chiesto per la sezione rifatta di era residence di architecture»).** In home la foto che si apre a schermo intero (le tende, lo stage .75 → 1: spec 2026-09-13 §3.10) porta il TITOLO del capitolo, «Open Domus», in Playfair bianco nudo (A40) alla misura di «ARCHITECTURE», `clamp(2.6rem, 12vw, 13rem)` con interlinea 0,85, centrato e appoggiato al bordo alto della foto (`.dt-od_titolo`, assoluto nella `.dt-od_cornice`, 0,05em dentro), sopra la foto nello stacking e sotto lo schermo avorio: compare quando il buco delle tende arriva in cima, cioè quando la foto riempie lo schermo, e scala con lo stage. La foto è la facciata a terrazze col glicine generata con Higgsfield (`villa-terrazze-glicine.jpg`, 3:2, `objectPosition: "50% 30%"`; sostituisce `villa-fronte-acqua.jpg`). Il corpo del capitolo sotto non ripete l'h2 (occhiello, claim, intro, liste e rilancio restano; su /metodo e /open-domus, senza finestra, l'h2 sta nel corpo come prima). Sul telefono lo stesso titolo a 46,8 px su una riga in cima al quadrato.
...
### Do:
- **Do** tenere un solo fondo, #f9f5ef, dalla testata al footer; le sezioni si distinguono per vuoto (`clamp(4.5rem, 10vh, 8rem)`) e hairline, non per colore.
...
- **Don't** mettere lettere sopra un'immagine fuori dai punti dichiarati: lockup e firma dell'hero, la testa delle pagine interne dentro la foto alta (A38, bianco nudo, A40), il titolo «Open Domus» sulla facciata della finestra (A45), copertina delle cinque stelle. Sul video del Congedo nessuna lettera, nessun tasto, nessuna tinta né angolo (A35, D108, D111): la testa sta sopra, il comando nella fascia sotto.
