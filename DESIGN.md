...
- **Primo schermo delle pagine interne: «la testa di era»** (`PageHero` + `PageHeroTesta`; A38, A40 e A41 di Alberto, 20 set.; lo usano 11 pagine, 9 con la foto della villa e 2 legali ferme). La fotografia è il fondo a schermo intero dal primo pixel, sotto la testata trasparente (`header[data-su-foto]`, nav e monogramma bianchi): un riquadro `.dt-testa_riquadro` alto 100svh, `position: sticky; top: 0`, `overflow: clip`, che resta FERMO mentre il blocco dei testi e poi la pagina le scorrono sopra (A41: «lo scroll continuo senza uscire dalla foto»; nessun tuffo, nessuna parallasse, nessun corridoio: `page-dive`, `soglia` e `ingresso` del commit R sono morti). Il blocco `.dt-testa_blocco` sta in flusso con `margin-top: -100svh`, alto almeno 100svh, griglia a tre righe centrate come «Perfect sea views» di era-residence: il lead in alto sotto la testata, occhiello + h1 + calligrafia al centro, i due comandi in basso (bottone rosso pieno, link fantasma bianco). L'h1 ha una misura sua, `clamp(3rem, 9vw, 9rem)` con interlinea 0.92, da lg `clamp(3.5rem, 7vw, 8rem)`, in colonna ≤ 18ch; la calligrafia è bianca. I tre punti di prova seguono sull'avorio (`.dt-testa_pagina`, alta ≥ 60svh) e salgono sopra la foto. L'inquadratura (`objectPosition`) è scelta per fascia in `tinte.json` (`lg` / `sotto`, D180) con A27 prima di tutto: nessuna persona tagliata. Il layout è tutto in CSS prima del paint: con reduced-motion e senza JS la pagina è identica. **Deroga di Alberto a WCAG 1.4.3 (A38 + A40, «bianco nudo»)**: le scritte bianche stanno sulla sola fotografia, senza ombra, alone, velo o tinta (A40: «negli screenshot di era-residence non c'erano le ombre sulle scritte bianche, rimuovile subito»). Misurato sui pixel resi (`qualita/a38/inquadrature.md`, 25 inquadrature × 3 viewport): otto rotte su nove non arrivano a 3:1 — h1 1,3-2,0:1, lead 1,1-1,8:1 — e passa solo /metodo (h1 4,17:1); è una scelta dichiarata di Alberto, da mostrare alla cliente, non un difetto da correggere con un velo (C14). Prima che la foto decodifichi il riquadro mostra la tinta alta della rotta (D125); l'e2e di accessibilità aspetta la foto prima di misurare.
...
- **Don't** mettere lettere sopra un'immagine fuori dai punti dichiarati: lockup e firma dell'hero, la testa delle pagine interne dentro la foto a schermo intero (A38, bianco nudo, A40), copertina delle cinque stelle. Sul video del Congedo nessuna lettera, nessun tasto, nessuna tinta né angolo (A35, D108, D111): la testa sta sopra, il comando nella fascia sotto.
