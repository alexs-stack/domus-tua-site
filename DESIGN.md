...
- **Avorio profondo** (#f2cfc5): la testata del telefono quando scorre (con hairline sotto) e il rettangolo sotto ogni foto e video mentre carica (la scatola dell'hero è la carta: il cielo è trasparente). Non è un secondo fondo di sezione.
...
### Hierarchy
- **Hero / lockup** (800, `clamp(3.1rem, 13vw, 13rem)`, 0.9, tracking −0.02em, `--font-brand`): il solo wordmark «Domus Tua» delle pagine, sull'acqua della foto alta della home (A49). Sta su due righe centrate, «Domus» in grafite e «Tua» in rosso, in maiuscole e minuscole come il logo. Non è un h1: è un `div`, ed è per questo che sfugge al maiuscolo. La tipografia del lockup è rimasta quando le posizioni sono tornate quelle di prima (Alberto, 10 set.: «lascia il nuovo font pero … intendo solo le posizioni», dove il font nuovo è la tipografia della rivista bianca, non un font consegnato). Il lockup del preloader invece è in Playfair 500 a 11vh (`font-hero`): se allinearlo già ora a `--font-brand` o aspettare il logo nuovo è una domanda aperta.
...
- **d3** (500, `clamp(1.5rem, min(4.5vh, 2.8vw), 2.6rem)`, 1.05; sotto i 1024 px `clamp(1.5rem, 4.8vw, 2.6rem)`): l'H1 dell'hero (sulla foto da lg, in bianco a d3 × 1,2; ≤ 28ch), la citazione della fondatrice (in tondo), i sei nomi sotto la rotaia del team, i titoli dei servizi, i titoli in mezza colonna.
...
- **Capitolo** (`.dt-chapter`): padding verticale `clamp(4.5rem, 10vh, 8rem)` (sotto i 768 px `clamp(2.5rem, 6vh, 4rem)`); la finestra di Open Domus non ha passo in alto: la foto comincia col cielo, che è la carta (A57). Era 14vh come nel riferimento, che però ha sei capitoli: sui diciassette della home sommava 3.800 px di avorio vuoto a 1440×900 (Alberto, 20 set.: «la home è troppo lunga … riducendo la distanza tra una sezione e l'altra»). Ogni sezione della home è `dt-chapter` su avorio; nessuna banda cambia fondo.
...
**La finestra di Open Domus è un nastro come «Perché Domus Tua» (A57 e A58 di Alberto, 22 set., sera: «aggiungere lo stesso effetto di scroll orizzontale con gsap e animazione entrata immagine come in "Tra la Pineta e Milano" nella sezione Open domus … all'altezza dello screenshot allegato»; «togli l'animazione dell'immagine di open domus all'entrata, mantieni la stessa posizione ma lasciala a schermo intero da subito, non serve neanche l'animazione di uscita che si chiude»).** In home `OpenDomus` monta `HorizonScroller` (`#open-domus`, `data-corridor="finestra"`) con tre pannelli di 100vw: (1) la cornice della facciata che sale, `villa-facciata-sale-alta-cielo.webp` 9:16 INTERA (`--dt-od-ar`, `sizes` 100vw, il cielo trasparente è la carta) col titolo «Open Domus» in inchiostro a 12vw appoggiato al bordo alto (`.dt-od_titolo`); (2) il claim del format in d2 per lettera (A66) con l'occhiello, l'intro in misura lead e il video di Teresa nella metà 16:9, a sipario; (3) la CODA (A66, A67, la stessa sera): la piscina lunga no-bg scelta da Alberto (`villa-piscina-lunga-alta-cielo.webp`, 2:3, misure in `app/lib/motion/coda.json`) a tutta larghezza, a sipario, e sopra di lei, sulla carta del cielo, il titolo a gradini («Per chi vende / e per chi / compra», tre righe in parallasse contraria come «Tra la Pineta e Milano»), le due liste in misura lead e il rilancio. Da 1024×640 con motion ok lo schermo si aggancia con la foto a schermo intero DA SUBITO — niente tende, niente stage in scala — e la scatola della foto SALE dentro lo schermo mentre il titolo resta fermo (A65; `lead` di HorizonScroller; finestra.ts `leadDistance` = `cima × altezza − metà titolo`, lineare, 1:1) finché il tetto delle terrazze non passa dietro le lettere, a metà della loro altezza; poi il track scorre di lato con la firma del capitolo (dtInOut, scrub 0,15: D18 vieta di copiare le cifre di storia) e la foto esce a sinistra; la coda arriva già alzata (il soggetto al 55 % dello schermo) e, finito il track, foto e scritte scendono 1:1 con lo scroll (`tail`) finché il fondo della piscina non tocca il fondo dello schermo: lì la sezione finisce, e all'uscita la piscina si ritira nella cornice della cartolina come le teste e il Congedo (A68, 8/22, dtCartolina). La sezione è alta salita + corsa del track + coda + schermo. I sipari sono quelli del nastro (dtOut 1,6 s, scala 1,15→1), i testi dei pannelli sono gruppi del motore dei reveal, e non c'è chiusura in cartolina. Sotto la soglia, con reduced-motion e senza JS: tre pannelli in colonna, la foto intera a tutta larghezza, il capitolo dopo di lei in inchiostro. Le bande del segno (`.dt-od_soggetto`, `foto`: le travi delle pergole) viaggiano con la cornice; sul cielo-carta e sui muri il segno resta grafite. La rete di fuoco dei nastri (storia compresa): il Tab su un focalizzabile di un pannello fuori scena porta la pagina alla quota in cui quel pannello è in scena, invertendo l'ease del track. Su /metodo e /open-domus il capitolo resta senza nastro (D28).

**Costi chiari è il secondo nastro con la salita (A72 di Alberto, 22 set., notte: «togliamo il video della piscina, e mettiamo un'altra immagine no-bg alta: villa-facciata-piscina-alta-cielo.webp, stesso stile e animazione dello sticky scroll che poi diventa scroll orizzontale, ed entra la sezione di Carmine e Seguici»).** In home `CostiChiari nastro` monta `HorizonScroller` (`#costi`, `data-corridor="costi"`) con tre pannelli di 100vw: (1) la facciata della villa bianca con la piscina (`villa-facciata-piscina-alta-cielo.webp`, 2:3 INTERA, `--dt-cc-ar`, misure in `app/lib/motion/costi.json`, il cielo trasparente è la carta) e sopra il cielo, A DESTRA, il solo TITOLO in inchiostro per lettera (`.dt-cc_titolo`, 7,5vw, due righe su 52vw) — D-A72-1, rivista la stessa notte dopo Alberto («non c'è lo sticky scroll che fa salire un po' l'immagine»; «le scritte e i bottoni sono troppo piccole e non animate»): a destra perché a sinistra ci sono i cipressi (fino al 36 % della larghezza) e l'inchiostro non vi si legge, mentre sotto il titolo il cielo scende fino al tetto piatto; niente bianco sulla foto (A70): la villa è bianca; (2) il CLAIM dei costi, come il secondo pannello della finestra ma largo 60vw (testo solo: su 100vw restava mezza carta vuota, la lezione del manifesto del 20 set.): occhiello, la prima frase del lead in d2 per lettera («Si paga solo a vendita conclusa.»), il resto in misura lead, le due righe del mandato e il rilancio PIENO dell'hero (`cta-solid`, `lg`); (3) Carmine (`FeaturedTestimonial panel`): la cornice del video nella metà 16:9 a sipario, e dentro la foto che affonda del 10 % mentre il pannello attraversa lo schermo (la firma di `testimonianza`, dtAffonda 1,2, agganciata al track con `useHorizonTrack`, il canale del track di HorizonScroller); (4) Seguici (`Social`): occhiello, titolo per lettera a 5,2vw, lead e icone, col congedo del titolo (A25) allo sgancio dello schermo. Da 1024×640 con motion ok lo schermo si aggancia con la facciata a schermo intero e il titolo sul cielo; la scatola della foto SALE — l'ARRIVO, `costi.ts` `arrivoDistance` = `cimaTitolo (0,332, il tetto sotto il titolo) × altezza − metà del titolo`, lineare, 1:1, la salita di A65 — finché il tetto piatto della villa non passa dietro le lettere, a metà della loro altezza (≈ 450 px a 1440×900, ≈ 300 a 1024×768: il titolo, Playfair maiuscolo a 7,5vw su 52vw, va su tre righe); poi il track scorre di lato con la firma del capitolo (dtTappe, una curva a due soste: il track rallenta a una deriva lenta dove il claim è al centro dello schermo e dove Carmine è al centro, poi si posa su Seguici — con una curva in-out il track correva proprio sui due pannelli coi contenuti, e Carmine restava intero nello schermo per il 2,5 % della corsa, ora per il 17,4 %; scrub `true`, il solo track saldato allo scroll: la morbidezza è quella di Lenis; D18). Niente coda né cartolina (D-A72-3: la coda della finestra resta la sola, l'hero e il Congedo si chiudono già in cartolina): finito il track lo schermo si sgancia e la sezione, alta arrivo + corsa + schermo, sale con la pagina. La banda dell'acqua (spec §3.13, D25) è uscita dal codice; i file restano in `public/media`. Sotto la soglia, con reduced-motion e senza JS: il titolo in flusso prima della foto (la carta del cielo lo continua), la foto intera, il claim, Carmine e Seguici in colonna. Le bande del segno (`.dt-cc_soggetto`: i cipressi, il gelsomino e la vetrata, la piscina) viaggiano con la foto. Su /vendi resta la riga sola; /vendi, /acquista e /recensioni tengono il blocco di Carmine con la lama.

*(Superato la sera del 22 set. da A57/A58, qui sopra: tende, stage, pista, marcatori da 1 px, capitolo sulla foto e chiusura non esistono più; il paragrafo resta come storia.)* **La finestra di Open Domus era la sezione «Architecture» di era-residence (A45 di Alberto, 21 set.: «non hai fatto quello che ti ho chiesto per la sezione rifatta di era residence di architecture»; A46, 21 set., sera: il cielo mascherato).** In home la foto che si apre a schermo intero (le tende, lo stage .75 → 1: spec 2026-09-13 §3.10) porta il TITOLO del capitolo, «Open Domus», in Playfair alla misura di «ARCHITECTURE», `clamp(2.6rem, 12vw, 13rem)` con interlinea 0,85, centrato e appoggiato al bordo alto della foto (`.dt-od_titolo`, assoluto nella `.dt-od_cornice`, 0,05em dentro), sopra la foto nello stacking e sotto lo schermo avorio: compare quando il buco delle tende arriva in cima, cioè quando la foto riempie lo schermo, e scala con lo stage. **Dal 21 set. (A46) il titolo è in INCHIOSTRO (`--color-ink`), scuro sul cielo che è il fondo pagina, come «ARCHITECTURE» negli screenshot di Alberto**; il bianco nudo di A40 valeva per il cielo fotografato. **Dal 22 set. (A47 di Alberto: «qua perchè hai tagliato l'immagine, deve continuare, abbiamo fatto le immagini alte apposta per poterci scrollare a schermo intero senza uscire dalla foto»; «questa sezione va sopra l'immagine di open domus») la foto è la facciata che SALE, `villa-facciata-sale-alta-cielo.webp` (il corridoio 9:16 di Higgsfield, 2160×3870, col cielo trasparente da `scripts/media/cielo.mjs`; misure in `app/lib/motion/finestra.json` da `scripts/media/finestra.mjs`: cima 0,246, bande del segno sulle travi), INTERA a ogni larghezza**: la scatola `.dt-od_window` ha il rapporto della sorgente (`--dt-od-ar`) e da lg riempie la cornice, che ha lo stesso rapporto; il cover non ritaglia nulla (`objectPosition: "50% 0%"`, `sizes` 100vw). Nel corridoio lo schermo agganciato mostra il primo schermo della foto (cielo, titolo, la terrazza alta) mentre le tende si aprono; dopo la pista di 200svh lo stage scorre via e la facciata prosegue sotto la piega, la sezione «Architecture» di era. **Il capitolo di Open Domus sta DENTRO la cornice, dopo la foto, ed è lo spazio sopra la foto delle teste (A48/A54)**: da lg comincia al 55 % dell'altezza della foto (`FINESTRA.sopra`, nel CSS `--dt-od-sopra` = 0,55 × h / w), nel grigio del lockup, senza ombra (A56, 22 set. pomeriggio; fino ad A56 in bianco con l'ombra di ink-media.ts) e coi corpi alla misura del lead; se il capitolo è più alto della foto la cornice cresce e la foto copre dalla cima; sotto lg il capitolo segue la foto in inchiostro. La finestra posa sulla carta (`--color-cream`) e a schermo intero gli angoli alti mostrano l'avorio della pagina. I marcatori del segno: `.dt-od_mark--f` è `foto-chiara` da +150svh fino al fondo dell'area (cielo, muri bianchi, travertino: grafite) e le travi scure delle pergole hanno i loro `.dt-od_soggetto` con `data-bg="foto"` (bande di finestra.json, come `.dt-testa_soggetto`). A fine coda la facciata si ritira nella cornice della cartolina (ChiusuraFoto, A53: Alberto, 22 set. pomeriggio, «deve rimpicciolirsi alla fine»), se sotto il capitolo resta almeno un quarto di viewport di foto. Il corpo del capitolo non ripete l'h2 (su /metodo e /open-domus, senza finestra, l'h2 sta nel corpo come prima). Sul telefono lo stesso titolo a 46,8 px su una riga in cima alla foto 9:16, sull'avorio del cielo, e l'otturatore di spec §3.10 apre la foto intera. La 3:2 col glicine (`villa-terrazze-glicine.jpg`) resta scena di riserva, senza WebP.
...
- pulsante di ricerca da 56 px (48 sotto lg), il punto fermo della domanda di /acquista (A80): col centro sulla riga del campo, a vuoto anello d'inchiostro da 1 px, con una frase disco rosso che si riempie (scala) e freccia bianca;
...
- **La domanda** (A80 di Alberto, 23 set.: «la ricerca intelligente non si vede … ingegnati e stupiscimi cambiando il design dell'input della ricerca, con qualcosa di figo e bello e consono al design del sito»). Il campo in linguaggio naturale di /acquista è l'eccezione dichiarata a «testo body, riga rossa 1 px», per questo campo solo: una riga in Playfair 400 alla misura di una testata (`clamp(1.375rem, 5.4vw, 1.75rem)` sotto lg, `clamp(1.5rem, 2.4vw, 2.625rem)` da lg: 22 px a 390, 34,6 a 1440, 42 a 1920), il testo scritto in tondo inchiostro, il segnaposto in corsivo pietra, il cursore rosso; da lg centrata sull'asse del titolo (il rientro a sinistra vale il pulsante più il suo spazio), sotto lg a sinistra con un segnaposto corto (`nlPlaceholderBreve`). La riga sotto resta d'inchiostro da 1 px; al fuoco una riga rossa da 2 px ci si disegna sopra (scaleX, entra da sinistra in 750 ms, esce verso destra in 350 ms, `--ease-domus`), ed è l'indicatore del fuoco al posto dell'anello. Il pulsante tondo sta col centro sulla riga, come il punto fermo della frase (vedi Shapes). La prima volta che il campo entra nello schermo l'esempio del segnaposto si scrive da solo, una volta, dietro un cursore rosso (`RigaScritta.tsx`: clip-path e xPercent, al più ~3,3 s, mai in loop; niente con reduced-motion, con un campo già pieno, con `?q=` o al fuoco). Tutto su carta piatta: inchiostro 7,5:1, pietra 4,9:1, rosso cupo 6,1:1.
...
### I set piece pilotati dallo scroll
Tornati l'11 settembre, rifatti senza curve, veli, scuro né testo sotto i 16 px (Alberto, 11 set.). Superano in parte «eliminare tante animazioni e transizioni» (la cliente, 10 set.): tornano i due gesti pilotati dallo scroll, e restano tolte le transizioni di pagina, il cursore custom e il resto del WOW layer. Dal 22 set. (A57) i nastri sono due: «Perché Domus Tua» e la finestra di Open Domus (il paragrafo in Shapes), con la stessa meccanica e firme diverse.

**Storie vere, il video di Roberta (A60 di Alberto, 22 set., sera: «mettilo al posto della preview youtube, con la logica di audio che si attiva in automatico, e quando esci dalla sezione scrollando la pagina, si disattiva»).** Nella riga sotto la testa di «Perché Domus Tua» il video della storia è il file vero, 1080×1920 (`StoryVideo`, `public/media/open-domus-roberta-{1080,720}.{mp4,webm}` con poster), nella colonna 9:16 a 36vw, 560 px al massimo (A63, «rimpicciolisci di poco»; il tetto a 420 px era il conto della copertina YouTube): parte in vista col suono, secondo la logica di A44 (se il browser nega l'autoplay col suono parte muto e si accende al primo gesto), si ferma fuori vista, anche sul telefono; due comandi da 56 px in basso a sinistra (pausa, audio), rossi pieni; chi toglie l'audio lo tiene tolto. Con reduced-motion il video non parte da solo e i comandi restano.

**Domus D.O.C., il foglio delle foto (A59 di Alberto, 22 set., sera: «aggiungere animazione e rendere più bella la sezione D.O.C., mettendo più foto»).** I cinque pilastri stanno in colonna a sinistra, a passo doppio (`py-12`) e con la riga in misura lead, così fra uno sfoglio e l'altro passano ~220 px di scroll (A64: «troppo veloce»); a destra una cornice (`.dt-doc_cornice`, da lg sticky a 10svh, larga `min(42vw, 640px, 80svh × 2/3)`) tiene cinque foto, una per pilastro (studio, ingresso con la scala, Raffaela nel soggiorno, il salotto doppio, Raffaela sotto il portico). Il pilastro che attraversa il 62 % dello schermo è quello attivo: il suo trattino rosso cresce verso il margine, gli altri pilastri scendono in pietra, e la sua foto entra a tendina (dal basso scendendo, dall'alto risalendo) con la scala 1,08→1 in 1,1 s dtOut mentre la cornice prende l'altezza del suo sorgente: nessuna foto è tagliata. Il sigillo D.O.C. è un anello di testo a 16 px che gira in senso orario (24 s a giro), con la sigla in Playfair al centro. Sotto lg la cornice sta sopra la lista, in flusso; con reduced-motion o senza JS resta la prima foto, ferma, e i pilastri in grafite. È una cosa sticky in più oltre i nastri: deroga dichiarata, da mostrare ad Alberto.
...
- **Don't** aggiungere sezioni sticky oltre i nastri registrati (la cornice delle foto di D.O.C., sticky da lg dal 22 set., A59, è la deroga dichiarata), né usare il pin di GSAP, né transizioni di pagina, cursori custom, fiori o ornamenti disegnati: il sito deve restare bello anche fermo.
