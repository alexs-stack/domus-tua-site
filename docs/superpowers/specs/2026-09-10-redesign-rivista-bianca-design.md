...
| A73 | 2026-09-22, notte, vedendo l'hero alto sul dev | «la foto della hero va modificata, usa Higgsfield per togliere il bg (ha qualche problema), e mettere la foto di Raffaela (attualmente è irriconoscibile perché modificata da Higgsfield). Usa quella del preloader. È come se dobbiamo photoshoppare la foto aggiungendo perfettamente la sagoma mascherata di Raffaela nella foto al posto di quella attuale. Proviamo con Higgsfield se fa. Non devono cambiare né rapporti, grandezza, né qualità» | **Applicata** (commit A73 del 22 set. 2026, notte; hash nel handoff): un fotoritocco, non una generazione (`scripts/media/hero-raffaela.mjs`): Higgsfield ha fatto solo il PLATE (gpt_image_2_5, «remove the woman», 2k, 3 crediti: job ef80fd4b), allineato e usato solo nelle schegge della figura alterata che la figura nuova non copre; la Raffaela del preloader (`raffaela-sagoma.png`, il ritaglio vero in pizzo) è posata a scala e posizione della figura vecchia (testa→ginocchio 352 px), sotto restano i pantaloni e le scarpe della foto; la JPEG resta 2560×3812 (ricodifica q92 4:4:4). Il «problema del bg» era la tasca di cielo pallido fra le fronde dell'albero a destra: cielo.mjs la libera con `raggio 700`, `libere 0,42` e il nuovo `pallido` (nessun credito di Higgsfield per lo sfondo). hero.json e la striscia 9:16 rigenerati (cima 0,194; `--dt-hero-cima`). Crediti Higgsfield: 2. | transcript 2026-09-22 | transcript 2026-09-22 |
