...
> **Stato al 2026-09-13.** Il piano è stato eseguito per intero il 2026-09-10 (da cf52a88 a 77d70e7, PR #79). I task restano scritti com'erano: raccontano come il redesign è stato costruito, non il codice di oggi. Il sistema attuale è in `DESIGN.md` alla radice; chi ha chiesto cosa, e quando, lo registra la spec (`docs/superpowers/specs/2026-09-10-redesign-rivista-bianca-design.md`) in §11, dove rimandano gli ID qui sotto, con le domande aperte in §11.3. Prima di riprendere un task va letta questa lista.
>
> - **Task 1-2, token e tipografia** (b9e6b7f): lead a 38ch, interlinee d1 0.98 e d2 1, eyebrow a 500 e 0.12em, oro #d9a441 e corsivo a -0.2em sono decisioni di lavoro dell'11 settembre (D08, D11); l'inchiostro è #46423d (A11).
> - **Task 4, preloader** (456026a): `TEMPO = 1` regge (C05, A02). `spinMarkBadge` è stato rimosso in 6b11620; il verso orario arriva al badge del preloader con c1ba870 (C06); l'ingresso è stato ripristinato col patto della porta in 6e6559b (A16, D06).
> - **Task 6, testata** (769bb64): in flusso, sticky solo sotto lg e senza CTA desktop con 7d2d1d8 (D14); su una riga con sei voci in 6e6559b (A13, D02).
> - **Task 9 e 12, ordine della home e «Come lavoriamo»** (3d4bd53, 2e94319): con 024d354 tornano `HorizonStory` e `StarReviews` e `ComeLavoriamo.tsx` è cancellato (A12); la home monta `<Method compact />` (D10); la foto aerea torna nel pannello del territorio (D07, domanda aperta).
> - **Task 10, hero** (deb1291): il blocco CTA a destra, costruito per il punto 7 della cliente (C11), è stato ribaltato la sera stessa da Alberto, «rimettilo centrale» (A04, e6e34bd); dal giorno dopo la foto sta dietro la scritta (A10, 09aff4b) e sopra ci sono solo lockup e firma (D01).
> - **Task 13, voci** (44d3a6c): diventa `#voci`, senza il titolo col voto (360c76b), con le copertine rifilate (D13) e il conteggio a 542 (D12).
> - **Task 14-17, percorsi, metodo, Open Domus, servizi, costi e testimonianza** (f3e4b75, 2285b7d, d9bad48, 1cd9b96): un solo template a due colonne e i tre moduli media (D03), i nove passi solo su /metodo (D10), servizi numerati e Open Domus in video (4f6f4ea), la testimonianza sul video reale di un cliente (c122bf4).
> - **Task 18, rotaia del team** (cf9fe39): foto di gruppo vere al posto dei monogrammi (c122bf4), in tre tessere 4:5 (360c76b).
> - **Task 19-23, contatti, congedo, PageHero e pagine interne** (0dc9902, 3d4bd53, 75f0960, ab5cf6d): contatti su due colonne (a6f2bf2); `INK_ON_VIDEO` sul Congedo e testa delle pagine interne su due colonne (D15); `EditorialRows` senza foto di repertorio (D10).
> - **Task 24-25, cancellazioni e test** (3cee992, 0487185): tornano i set piece e le loro prove e2e (024d354); `Stats.tsx` resta, perché lo usano /chi-siamo e /recensioni.
...
- **Nessun raggio** tranne icon-button rotondi (play, frecce, WhatsApp): vietato `rounded-*` su figure, card, campi, bottoni; i token `--radius-*` valgono 0. *(Le curve rimaste oggi, dentro e fuori questa eccezione, hanno l'inventario unico in `DESIGN.md`, sezione Shapes. Il disco carta sotto il badge del preloader è una domanda aperta: spec §11.3.)*
- **Nessuna ombra**, nessun gradiente sopra le foto, nessuna grana, nessuna vignettatura. *(Le eccezioni costruite dopo il piano sono elencate per intero in `DESIGN.md`, Elevation & Depth, e solo lì: un elenco parziale qui si leggeva come completo. A riposo sopra le foto non si vede nessuno strato: quello del lampo delle cinque stelle, `.dt-starrev_flash`, resta a opacity 0 e GSAP lo accende solo nel lampo.)*
- **Nessun testo sotto 16 px** nel contenuto: vietati `text-xs`, `text-sm`, `text-[0.6…0.9rem]`. Corpo 19 px (`text-body`), paragrafo editoriale `lead`, UI 16 px (`text-ui`). *(Oggi non la rispettano il preloader (domanda aperta, spec §11.3) e la mappa della ricerca: l'inventario completo è in `DESIGN.md`, «La regola dei 16 px».)*
