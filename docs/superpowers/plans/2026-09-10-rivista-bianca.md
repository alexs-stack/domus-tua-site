...
> **Stato al 2026-09-13: cosa è cambiato dopo questo piano.** Il piano è stato eseguito per intero il 2026-09-10 (da cf52a88 a 77d70e7, PR #79). I task restano scritti com'erano: raccontano come il redesign è stato costruito, non il codice di oggi. Nei giorni successivi direttive di Alberto e decisioni prese costruendo ne hanno ribaltati o cambiati diversi. Prima di riprendere un task va letta questa lista. Il sistema attuale è in `DESIGN.md` alla radice. Chi ha detto cosa e quando lo registra la spec (`docs/superpowers/specs/2026-09-10-redesign-rivista-bianca-design.md`) in §11, «Direttive arrivate dopo la costruzione»: §11.1 ha le direttive con le parole esatte, §11.3 le domande aperte.
>
> - **Task 1-2, token e tipografia** (b9e6b7f). aff9b0e (2026-09-11, decisione di lavoro) porta `.lead` a 38ch, d1 a interlinea 0.98 e d2 a 1, l'eyebrow a tracking 0.12em e peso 500, l'oro delle stelle a `#d9a441`. 3411a55 porta il rientro del corsivo a -0.2em. 09aff4b porta l'inchiostro da `#1a1816` a `#46423d` («NIENTE SCRITTE BLACK», Alberto, 2026-09-11).
> - **Task 4, preloader** (456026a). `TEMPO = 1` regge, 4,63 s: la cliente chiedeva «preloader più veloce», Alberto ha scelto «stesso film di oggi ma dimezzato» (2026-09-10). Del passo 3 è caduto solo `spinMarkBadge`, rimosso in 6b11620. Il resto regge: `RotatingMark` in testata gira in senso orario a 30°/s e con lo scroll accelera senza invertirsi mai (il cuore orario è il punto 2 della cliente, 2026-09-10). Il verso orario arriva al preloader in CSS con c1ba870, dove però il giro dura 6 s e non 12. L'ingresso è stato ripristinato con 6e6559b dopo che Alberto l'ha rivoluto «come prima» (2026-09-11): `--dt-head-h` e `--dt-band-h` mettono la sagoma del sipario e la banda dell'hero nella stessa scatola.
> - **Task 6, testata** (769bb64). 7d2d1d8 (2026-09-10, decisione di lavoro) la toglie da `fixed`: sticky solo sotto lg, da lg scorre via, e sul desktop non ha più la CTA. 6e6559b (2026-09-11) la mette su una riga con sei voci primarie, `RotatingMark` a h-14 solo da xl e il logo a `w-[clamp(150px,13vw,210px)]`. Il «menu sopra» l'ha indicato Alberto, mentre il commit lo attribuisce al cliente; le sei voci sono una decisione di lavoro.
> - **Task 9 e 12, ordine della home e «Come lavoriamo»** (3d4bd53, 2e94319). Con 024d354 (Alberto, 2026-09-11) tornano `HorizonStory` e `StarReviews`, e `ComeLavoriamo.tsx` viene cancellato: il suo copy passa a `HorizonStory`. Con 0fa94bc la home monta `<Method compact />`. Il contratto in testa a `page.tsx` non è più quello del passo 1. La prima schermata ha le CTA centrate e nessun video sotto il titolo, e il paragrafo FORM dà al cliente il ritorno dei set piece, che 024d354 attribuisce ad Alberto. 360c76b (decisione di lavoro) rimette `/media/hero-aerial.jpg` nel pannello del territorio, contro il punto 6 della cliente («togliere foto dopo ricerca»). c2949a3 (2026-09-13) ne corregge la descrizione, perché di quella villa non si conoscono né il proprietario né i diritti.
> - **Task 10, hero** (deb1291). Il punto 7 della cliente, «box vendi casa più a destra» (2026-09-10), è stato costruito come blocco CTA a destra; che «box» fosse quel blocco lo dice solo la spec. La sera stessa Alberto ha scritto tre messaggi. «nella hero hai cambiato le posizioni non mi piace, rimettilo centrale» (20:54Z) ribalta il punto 7. «hai cambiato tutto, la hero rimettila con la foto sotto la scritta» (20:55Z) chiede la foto sotto il testo. «lascia il nuovo font però, … intendo solo le posizioni» (20:56Z) tiene la tipografia della rivista. e6e34bd li esegue tutti e tre: lockup, firma, sovratitolo, H1 e CTA centrati, poi il video a tutta larghezza sotto. Il giorno dopo Alberto supera il secondo messaggio con 09aff4b («la foto nella hero va messa dietro la scritta Domus Tua»): la foto diventa una banda di 60svh senza velo. Che sopra ci stiano solo lockup e firma lo decide il lavoro, nello stesso commit, per leggibilità. La firma scavalca il bordo della banda (aff9b0e, poi `translate-y-[26%]` con fd07a63). L'H1 è `text-d3` a 28ch da 02d19d3.
> - **Task 13, voci** (44d3a6c). Con 024d354 diventa `#voci`, e `#recensioni` passa a `StarReviews`. 360c76b toglie il titolo col voto. c0d841f e 39cedc3 rifilano le copertine con `.dt-still-trim`. Con 98b8709 il conteggio è 542, non 531.
> - **Task 14-17, percorsi, metodo, Open Domus, servizi, costi e testimonianza** (f3e4b75, 2285b7d, d9bad48, 1cd9b96). 5304dfd dà a percorsi e metodo un solo template a due colonne, con gli atti in 16:9. Il commit parla della «posizione delle foto» come di un giudizio del cliente, ma quella frase l'ha scritta Alberto (2026-09-11). Con 0fa94bc i nove passi stanno solo su /metodo. 4f6f4ea fa dei servizi un elenco numerato 01-06 con una foto per riga, e dà a Open Domus un video al posto del fotogramma quadrato. Con c122bf4 la testimonianza è il video reale di un cliente, non più una citazione.
> - **Task 18, rotaia del team** (cf9fe39: ritratti 4:5 e monogrammi). c122bf4 porta foto di gruppo vere e i sei nomi sotto la rotaia. 360c76b (decisione di lavoro) lascia tre tessere, tutte nella cornice `.dt-media-column` 4:5.
> - **Task 19, contatti** (0dc9902). a6f2bf2 passa a due colonne uguali e, su /contatti, non ripete la testa della pagina.
> - **Task 20 e 22, congedo e PageHero** (3d4bd53, 75f0960). 7d2d1d8 toglie al titolo del congedo l'ombra prevista qui. 8b21627 gliene rimette una più forte, `INK_ON_VIDEO`, e porta la testa delle pagine interne su due colonne. 24e1d06 cambia la parola in corsivo delle pagine interne, che ripeteva l'occhiello.
> - **Task 23, pagine interne** (ab5cf6d). Con 183acdd `EditorialRows` mostra le foto solo se ogni riga ne ha una vera in `/reali/`, altrimenti è un elenco numerato: oggi lo è su tutte e quattro le pagine che lo usano. 7fe3694 porta il numerale a pietra. Il 531 del passo 1 è 542 (98b8709).
> - **Task 24-25, cancellazioni e test** (3cee992, 0487185). Con 024d354 tornano `HorizonStory`, `StarReviews`, `motion/HorizonScroller`, `lib/star-shape` e le loro prove e2e. `Stats.tsx` non è stato cancellato: lo usano /chi-siamo e /recensioni.
>
> **Aperto, da non decidere senza Alberto o la cliente** (l'elenco completo è in spec §11.3). Va detto alla cliente che il suo punto 7 è stato ribaltato? La foto aerea resta subito dopo la ricerca, contro il suo punto 6? I set piece e il «menu sopra» li ha voluti Alberto, o riferiva un giudizio della cliente? Il «come prima» del preloader comprendeva anche i 9,26 s di prima? Infine il preloader va contro quattro direttive della cliente, e nessuno ha deciso se restano eccezioni. Il pannello espresso va contro «eliminare nero ovunque» (2026-09-10): il vincolo qui sotto lo esclude, ma l'eccezione l'ha scritta la spec, non la cliente. Le didascalie a 0.68rem e 0.82rem vanno contro «nessuna scritta piccola». Il disco carta sotto il badge va contro «metti il logo senza sfondo bianco» (2026-08-06). Il lockup «Domus Tua» in Playfair, mentre quello dell'hero è in `--font-brand`, va contro «stesso font del logo in tutte le scritte Domus Tua».
...
- **Nessuna superficie scura** fuori dal preloader: vietati `bg-ink`, `bg-espresso`, `bg-wine`, `bg-graphite`, `from-ink/*`, `via-ink/*`, `text-cream` su fondo scuro. *(L'eccezione del preloader l'ha scritta la spec del 10 settembre (§3.1), non la cliente, che lo stesso giorno ha chiesto «eliminare nero ovunque». Se il pannello espresso resta è una domanda aperta.)*
- **Nessun raggio** tranne icon-button rotondi (play, frecce, WhatsApp): vietato `rounded-*` su figure, card, campi, bottoni; i token `--radius-*` valgono 0. *(Nel codice di oggi sono tondi anche i link social, `.dt-social__link` a `border-radius: 9999px`, e gli altri bottoni-icona di schede, galleria e assistente. È tondo anche il disco carta sotto il badge del preloader (`rounded-full bg-paper`), che va contro «metti il logo senza sfondo bianco» della cliente, 2026-08-06: se resta è una domanda aperta.)*
- **Nessuna ombra**, nessun gradiente sopra le foto, nessuna grana, nessuna vignettatura. *(Eccezioni costruite dopo il piano. Un'ombra attaccata alle lettere bianche sopra le immagini: il titolo del Congedo con `INK_ON_VIDEO` (8b21627) e la parola della copertina delle cinque stelle. Un alone dietro il nome «Raffaela Rizza» in corsivo, sotto il lockup del preloader. L'ombra all'hover dei link social tondi. Sopra la foto delle cinque stelle resta inoltre uno strato a tutta superficie, `.dt-starrev_flash`: a riposo è a opacity 0 e GSAP lo accende nel lampo. Il suo commento lo chiama velo e parla ancora di un «velo di vino», che 024d354 ha tolto.)*
- **Nessun testo sotto 16 px** nel contenuto: vietati `text-xs`, `text-sm`, `text-[0.6…0.9rem]`. Corpo 19 px (`text-body`), paragrafo editoriale `lead`, UI 16 px (`text-ui`). *(Il preloader oggi non la rispetta: in `PreloaderShell.tsx` le didascalie sono a `text-[0.68rem]` e il payoff a `text-[0.82rem]`. Portarle a 16 px o dichiarare l'eccezione è una domanda aperta.)*
...
- Movimento ammesso: `Reveal`, `TextLines`, `Parallax` (±4 %), `HorizontalRail` (solo team). Vietati tutti gli altri primitivi di `app/components/motion/`. Reduced-motion: tutto fermo e visibile. *(Dal 2026-09-11 si aggiungono `HorizonScroller` e il film di `StarReviews`, rifatti nel design nuovo. Li ha chiesti Alberto l'11 settembre: «dovevamo fare un redesign, ma mantenendo quelle animazioni che non erano curve» (024d354). Se riferiva una richiesta della cliente, nessuna fonte lo dice.)*
