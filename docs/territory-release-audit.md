...
| 2c | E2E/A11y | full-site E2E, assistant E2E, accessibilità | Playwright configurato (`test:e2e`, `e2e:assistant`, `@axe-core`); **non eseguibile** senza preview/browsers qui | **BLOCKED** | Dev/QA | Eseguire in CI contro il preview deployato |
...
| 11 | **Territory** | Chatbot: separazione fattuale, provenienza, domande sensibili, fallback stale | `territoryAssistant.test.ts` (16) + eval gruppo `territorio` 8/8, tool selection 100% | **PASS (sim)** | Dev | Eval REALE con provider (GEMINI) come step opzionale |
