...
//
// Il check ignora i COMMENTI (una nota che spiega "il 92% è stato rimosso" non è un claim) e
// distingue due famiglie:
//   • `code`: cifre-firma delle vanity metric. Un 269395 nudo nel codice è sempre quel dato,
//     quindi si cerca in tutto il sorgente non commentato.
//   • `copy`: percentuali/conteggi che esistono anche come valori tecnici legittimi
//     (`start: "top 92%"` di ScrollTrigger, stop di un gradiente, `text-[0.92rem]`). Si cercano
//     SOLO dentro stringhe lunghe (≥ 40 caratteri): il claim di marketing vive in una frase,
//     il valore CSS in una stringa corta.
type Scope = "code" | "copy";
const BANNED: { re: RegExp; what: string; scope: Scope }[] = [
  { re: /\b269395\b/, what: "mq valutati inventati (269395)", scope: "code" },
  { re: /\b6433\b/, what: "persone felici inventate (6433)", scope: "code" },
  { re: /\b1523\b/, what: "transazioni inventate (1523)", scope: "code" },
  { re: /440\s*\+/, what: 'conteggio video non verificato ("440+")', scope: "copy" },
  { re: /\b92\s*%/, what: "% venduto non verificata (92%)", scope: "copy" },
];

/** Soglia oltre la quale una stringa è "una frase" e non un valore tecnico. */
const COPY_MIN_LEN = 40;

/** Rimuove commenti di linea e di blocco (approssimazione sufficiente per sorgenti TS/TSX). */
function stripComments(src: string): string {
  return src.replace(/\/\*[\s\S]*?\*\//g, " ").replace(/(^|[^:])\/\/[^\n]*/g, "$1 ");
}

/** Stringhe letterali "lunghe" (copy leggibile) presenti nel sorgente. */
function copyStrings(src: string): string[] {
  const out: string[] = [];
  const re = /"((?:[^"\\]|\\.)*)"|'((?:[^'\\]|\\.)*)'|`((?:[^`\\]|\\.)*)`/g;
  for (const m of src.matchAll(re)) {
    const s = m[1] ?? m[2] ?? m[3] ?? "";
    if (s.length >= COPY_MIN_LEN) out.push(s);
  }
  return out;
}
...
for (const f of files) {
  ...
  const copyText = copyStrings(code).join("\n");
  for (const b of BANNED) {
    const haystack = b.scope === "code" ? code : copyText;
