 ...
 * immobili finisce serializzato nell'HTML per l'idratazione. Descrizione, galleria e i FATTI
 * strutturati non servono a una scheda della griglia — servono alla pagina dell'immobile, che
 * li carica per conto suo. La card mostra copertina, badge e i quattro numeri (mq/locali/camere/
 * bagni), e il filtro/ranking usa `features`/`excerpt`/`badges`, MAI `facts` (verificato:
 * app/lib/ai/rank.ts, PropertyCard, PropertySearch). Su 196 annunci reali `facts` è il campo più
 * pesante dopo descrizione e galleria (~300 kB non compressi di HTML per niente).
 */
export type GridProperty = Omit<Property, "description" | "gallery" | "facts">;
