...
// FrozenLines — i titoli di /case/[slug] (D32, A26 di Alberto; spec §5.4 «il movimento della
// pagina non cambia»): sotto MotionFreeze SplitTitle rende questo componente, con le righe in
// maschera dei titoli di prima di A20. Ogni riga sale da una maschera (SplitText, type lines +
// mask) e rigioca a ogni passaggio nei due versi (C22, cliente 2026-08-04): lo split resta vivo
// e `autoSplit` lo ricostruisce al resize. Il cambio lingua rimonta via key. Con reduced-motion
// o senza JS il testo è statico e visibile.
import { useRef, type CSSProperties, type ElementType, type ReactNode } from "react";
import { SplitText } from "gsap/SplitText";
import { gsap, ScrollTrigger, useGSAP, MQ } from "../../lib/motion/gsap";
import { useLocale } from "../i18n/LocaleProvider";

// SplitText si registra localmente, qui e in HorizonScroller, per non finire nel chunk del
// layout (che importa gsap.ts via SmoothScroll).
...
export default function FrozenLines({
  children,
  className = "",
  as: Tag = "div",
  id,
  ...
  return (
    <Tag key={locale} ref={ref} id={id} className={className} style={style}>
      {children}
    </Tag>
  );
}
