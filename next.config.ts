...
// Inventario redirect legacy: dato unico e VALIDATO (loop/catene/doppioni) in app/lib/seo/redirects.ts.
import { legacyRedirects } from "./app/lib/seo/redirects";
...
const nextConfig: NextConfig = {
  ...
  async redirects() {
    // Inventario legacy in app/lib/seo/redirects.ts (dato unico, validato da un test:
    // niente loop, catene o doppioni). Da riconciliare con gli export del cliente prima del
    // go-live — vedi docs/seo-redirects.md e `npm run redirect-audit`.
    // NB: le `source` NON portano lo slash finale (Next normalizza prima); `permanent: true`
    // emette 308, non 301 (equivalente ai fini della canonicalizzazione).
